...
function loadMetadata(query) {
	...
	net.apiCall(url, function(response) {
		...
		for (var i = 0; i < response.records.length; i++) {
			...
			var metadataView = Ti.UI.createView({
				left: 80,
				top: 5,
				layout: "vertical"
			});
			row.add(metadataView);
			for (var j = 0; j< visibleAttrs.length; j++ ) {
				//Ti.API.info(visibleAttrs[j]);
				metadataView.add(Ti.UI.createLabel({
					left: 0,
					text: (j == 0) ? response.records[i][visibleAttrs[j]] : visibleAttrs[j] + ": " + response.records[i][visibleAttrs[j]],
					top: 0,
					//height: (j == 0) ? 38 : 32,
					font: {
						fontSize: "13dp",
